...
the probabilities is shown based on a fitted model (from which the data sets combining the training/test sets with the predicted probabilities are obtained).
 After which the confusion matrices and model performances are obtained for the both the balanced training set and unbalanced test set. ---> This same process
  can be followed for ANY probabilistic model that where balancing has been applied to the training set (under sampling, over sampling or a combination).

For models fitted using SMOTE, the above probability correction cannot be applied. Rather, a confusion matrix and threshold-dependent performance metrics can be
 obtained based on applying the optimal threshold to determine the predicted class labels (covered in practical 5).
#----------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
# NOTE: the class labels for the target are now "Yes" and "No". Let's change them to 0 and 1 to use our generic code:

df[[target]] <- factor(ifelse(df[[target]] == "Yes", "1", "0"))

# check to make sure the change has been implemented:
summary(df[[target]])
